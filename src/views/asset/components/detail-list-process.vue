    ...
    <a-col :flex="1">
      ...
      <a-form :model="pagination" auto-label-width :label-col-props="{ span: 4 }" :wrapper-col-props="{ span: 18 }"
        ...
        <a-row :gutter="24">
          <a-col :span="6">
            <a-form-item>
              <a-input-search allow-clear="true" v-model="pagination.port" @search="initTableData"
                :placeholder="t('scan.record.host.process.port')" />
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
...
import { getAssetHostProcList } from '@/api/asset/search';
...
const tableColumns = reactive([
  {
    title: t('asset.search.detail.process.processName'),
    dataIndex: 'processName',
  },
  {
    title: t('asset.search.detail.process.bindAddress'),
    dataIndex: 'bindAddress',
    slotName: 'bindAddress'
  },
  {
    title: t('asset.search.detail.process.port'),
    dataIndex: 'port',
    slotName: 'port'
  },
...
const initTableData = async () => {
  const response = await getAssetHostProcList(
    id,
    pagination.value
  );
