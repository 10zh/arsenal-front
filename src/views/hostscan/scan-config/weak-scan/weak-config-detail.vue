<template>
  <div class="container">
    <Breadcrumb :items="[
      ...
      'menu.weak.hostscan.config.detail',
    ]" />
    <a-layout>
      <!-- 头部信息块start -->
...
import { getWeakHostScanConfigDetail } from '@/api/scan/scan-config';
...
const initScanConfigDetail = async () => {
  const configDetail: any = await getWeakHostScanConfigDetail(id);
  ...
  configDetailDataList.push({
    label: t('host.scan.config.weakName'),
