<template>
    ...
    <a-row>
          ...
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item field="status" :label="$t('host.scan.config.configName')">
                <a-input v-model="pagination.configName" :placeholder="t('host.scan.config.configName.input')"></a-input>
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item field="status" :label="$t('host.scan.config.engineName')">
                <a-input v-model="pagination.engineName" :placeholder="t('host.scan.config.engineName.input')"></a-input>
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item field="status" :label="$t('host.scan.config.templateName')">
                <a-input v-model="pagination.templateName"
                  :placeholder="t('host.scan.config.templateName.input')"></a-input>
              </a-form-item>
            </a-col>
          </a-row>
          <a-row :gutter="24">
            <a-col :span="8">
              <a-form-item field="status" :label="$t('host.scan.config.target')">
                <a-input v-model="pagination.target" :placeholder="t('host.scan.config.target.input')"></a-input>
              </a-form-item>
            </a-col>
            <a-col :span="8">
              <a-form-item field="status" :label="$t('host.scan.config.scanStatus')">
                <a-select v-model="pagination.scanStatus" :placeholder="t('host.scan.config.scanStatus.input')"
                  allow-clear :options="scanStatusOptions">
                </a-select>
              </a-form-item>
            </a-col>
          </a-row>
      ...
      <a-col :flex="'86px'" style="text-align: right">
        <a-space direction="vertical" :size="18">
          <a-button type="primary" default-checked style="margin: 0 10px" @click="initConfigList">
            <template #icon>
              <icon-search />
            </template>
            {{ $t('global.search') }}
  ...
  <a-table row-key="id" :columns="columns" :style="{ height: tableHeight + 'px' }" :bordered="false" :data="tableData"
    ...
    <template #scanPress="{ record }">
      <div class="progress-wrap">
        <a-progress status="success" style="margin-right: 2px" :show-text="false" :percent="record.lastScanStatic.scanStatus === 6 ? 100 : record.progressNums
          " />
        <icon-check-circle-fill v-if="record.lastScanStatic.scanStatus === 6" style="color: green" />
    ...
    <template #operations="{ record }">
      ...
      <a-popconfirm v-if="[2, 3].includes(record.lastScanStatic.scanStatus)"
        :content="t('host.scan.config.operator.scan.stop.ack')" @ok="stopHostScanEvent(record)">
        <a-button type="text" size="small" style="padding: 10px; color: #db0ac2">
          {{ $t('button.stop') }}
        </a-button>
      </a-popconfirm>
      ...
      <a-popconfirm :content="t('host.scan.config.operator.delete.ack') + record.configName + '?'
  ...
  <a-pagination class="paginationStyle" :total="pagination.total" :current="pagination.pageIndex"
    :page-size="pagination.pageSize" show-total show-jumper show-page-size @change="pageIndexChangeEvent"
...
const initConfigList = async () => {
  const response: any = await getWeakHostScanConfigPageList(pagination.value);
...
const startHostScan = async (record: any) => {
  const response: any = await weakHostScan(record.id);
  if (!response.success) {
    return;
  }
  // 重新刷新列表
...
const gotoScanConfigDetail = (record: any) => {
  router.push({
    path: '/hostscan/hostWeakScanConfigDetail',
    query: {
      configId: record.id,
    },
  });
...
const suspendHostScanEvent = async (record: any) => {
  const response: any = await suspendWeakHostScan(record.lastScanStatic.scanId);
...
const stopHostScanEvent = async (record: any) => {
  const response: any = await stopWeakHostScan(record.lastScanStatic.scanId);
  if (!response.data.operation) {
...
const resumeHostScanEvent = async (record: any) => {
  const response: any = await resumeWeakHostScan(record.lastScanStatic.scanId);
  if (!response.data.operation) {
