<template>
  <div class="container">
    ...
    <a-layout>
      <!-- 头部信息块start -->
      <a-card :title="t('menu.list.configDetail')" :bordered="false" ref="header">
        <!-- 返回按钮 -->
        <div class="back-btn">
            ...
            <a-space direction="vertical" size="large" fill>
              <a-descriptions :data="configDetailData" :align="{ label: 'right' }">
                <a-descriptions-item v-for="item in configDetailData" :key="item.label" :label="item.label + ':'">
                  <a-tag v-if="item.slotName" :color="getStatusColor(item.value)">{{ getStatusText(item.value)
                  }}</a-tag>
      ...
      <a-layout class="content-container">
        ...
        <!-- 左侧主机列表&单个扫描记录的详情 -->
        <a-layout-content>
          <configScanDetailRecordList :tableHeight="tableHeight"></configScanDetailRecordList>
        </a-layout-content>
      </a-layout>
    </a-layout>
  </div>
...
onMounted(async () => {
  ...
  const height =
    document.documentElement.clientHeight - header.value.$el.offsetHeight - 290;
