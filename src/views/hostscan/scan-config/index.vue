<template>
  <div class="container">
    ...
    <a-card :title="$t('menu.list.scan.config')">
      ...
      <a-row>
        <a-col :flex="1">
          <a-form :model="pagination" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }" label-align="left">
            <a-row :gutter="24">
              <a-col :span="6">
                <a-form-item field="status" :label="$t('host.scan.config.configName')">
                  <a-auto-complete v-model="pagination.configName" :data="autoCompleteData"
                    :placeholder="t('host.scan.config.configName.input')" @focus="
                      searchSingleField(
                        'host_hsc',
                        'host_hsc_cn',
    ...
    " />
              ...
              <a-col :span="6">
                <a-form-item field="status" :label="$t('host.scan.config.engineName')">
                  <a-auto-complete v-model="pagination.engineName" :data="autoCompleteData"
                    :placeholder="t('host.scan.config.engineName.input')" @focus="
                      searchSingleField(
                        'scan_se',
                        'scan_en',
    ...
    " />
              ...
              <a-col :span="6">
                <a-form-item field="status" :label="$t('host.scan.config.templateName')">
                  <a-auto-complete v-model="pagination.templateName" :data="autoCompleteData"
                    :placeholder="t('host.scan.config.templateName.input')" @focus="
                      searchSingleField(
                        'host_hst',
                        'host_hst_tn',
      ...
      <a-table row-key="id" :columns="columns" :bordered="false" :data="tableData" :pagination="false"
        ...
        <template #operations="{ record }">
          ...
          <a-popconfirm :content="t('host.scan.config.operator.delete.ack') +
            ...
            <a-button type="text" status="danger" size="small" style="padding: 10px">
              <template #icon>
                <icon-delete />
              </template>
              {{ $t('button.delete') }}
