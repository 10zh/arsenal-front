<template>
  <a-row ref="header">
    <a-col :flex="1">
      <a-form :model="vulnerabilityPagination" auto-label-width :label-col-props="{ span: 6 }"
        :wrapper-col-props="{ span: 18 }" label-align="left">
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item :label="$t('scan.record.vulnName')">
              <a-input v-model="vulnerabilityPagination.vulnName" :placeholder="t('scan.record.vulnName.input')" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item :label="$t('scan.record.ipv4')">
              <a-input v-model="vulnerabilityPagination.ipv4" :placeholder="t('scan.record.ipv4.input')" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item :label="$t('scan.record.ipv6')">
              <a-input v-model="vulnerabilityPagination.ipv6" :placeholder="t('scan.record.ipv6.input')" />
            </a-form-item>
          </a-col>
        </a-row>
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item :label="$t('scan.record.componentName')">
              <a-input v-model="vulnerabilityPagination.componentName"
                :placeholder="t('scan.record.componentName.input')" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item :label="$t('scan.record.port')">
              <a-input v-model="vulnerabilityPagination.port" :placeholder="t('scan.record.port.input')" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item field="section" :label="t('scan.record.potential')">
              <a-select v-model="vulnerabilityPagination.potential" placeholder="Please select" allow-clear>
        ...
        <a-row :gutter="24">
          <a-col :span="8">
            <a-form-item field="section" :label="t('scan.record.safe')">
              <a-select v-model="vulnerabilityPagination.safe" placeholder="Please select" allow-clear>
                <a-option value="1">{{ t('global.true') }}</a-option>
                <a-option value="0">{{ t('global.false') }}</a-option>
              </a-select>
            </a-form-item>
          ...
          <a-col :span="8">
            <a-button type="primary" default-checked style="margin: 0 10px" @click="initHostVulnerabilityData">
              <template #icon>
                <icon-search />
              </template>
              {{ $t('global.search') }}
  ...
  <a-table :columns="vulnerabilityTableColumns" :data="vulnerabilityData" column-resize :pagination="false"
    :bordered="{ cell: true }" @sorter-change="sortedChangeEvent" :style="{ height: tabsHeight + 'px' }">
  ...
  <a-pagination class="paginationStyle" :total="vulnerabilityPagination.total"
    :current="vulnerabilityPagination.pageIndex" :page-size="vulnerabilityPagination.pageSize" show-total
...
onMounted(() => {
  // 动态计算高度
  const height =
    document.documentElement.clientHeight - header.value.$el.offsetHeight - 300;
