<template>
  <div>
    ...
    <a-tabs type="rounded">
      <!-- 扩展信息start -->
      <a-tab-pane key="1" :title="t('leak.detail.extras')">
        <a-scrollbar style="height:calc(100vh - 550px);overflow:auto">
          <a-table :columns="columns" :data="detailMessage.extraList" :pagination="false" />
        </a-scrollbar>
      </a-tab-pane>
      <!-- 解决方案start -->
      <a-tab-pane key="2" :title="t('leak.detail.solutions')">
        <a-scrollbar style="height:calc(100vh - 550px);overflow:auto">
          <a-table :columns="solutionColumns" :data="detailMessage.solutions" :pagination="false">
            ...
            <!-- <template #superCedeList="{ record }">
              <p v-for="(item,index) in record.superCedeList" :key="index"></p>
            </template> -->
...
const columns = [
  {
    title: t('leak.detail.extras.name'),
    dataIndex: 'name',
  }, {
    title: t('leak.detail.extras.value'),
    dataIndex: 'value',
  }]
// 解决方案表格头信息
const solutionColumns = [
  {
    title: t('leak.detail.solutions.title'),
    dataIndex: 'title',
  }, {
    title: t('leak.detail.solutions.additionalInfo'),
    dataIndex: 'additionalInfo',
    slotName: 'additionalInfo',
    width: 400,
  }, {
    title: t('leak.detail.solutions.patch'),
    dataIndex: 'patch',
  }, {
    title: t('leak.detail.solutions.rollUp'),
    dataIndex: 'rollUp',
    slotName: 'rollUp',

  }, {
    title: t('leak.detail.solutions.superCedeList'),
    dataIndex: 'superCedeList',
    slotName: 'superCedeList'
  }, {
    title: t('leak.detail.solutions.workaround'),
