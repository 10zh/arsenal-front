<template>
  <a-card class="record-message">
    ...
    <a-row id="record-detail-progress-text" ref="process" :gutter="24">
      ...
      <a-col :span="3">
        <a-statistic :title="t('host.scan.config.vulnerabilityNumber')" :value="progressTextData.vulnerabilityNumber" />
      </a-col>
      <a-col :span="4">
  ...
  <a-card :style="{ height: props.tableHeight - 132 + 'px' }">
    <a-tabs type="rounded" @tab-click="changeClick">
      <a-tab-pane key="1">
        ...
        <a-pagination :style="{ marginTop: '20px', float: 'right' }" :total="hostPagination.total"
          :current="hostPagination.pageIndex" :page-size="hostPagination.pageSize" show-total show-jumper show-page-size
          @change="hostPageIndexChangeEvent" @page-size-change="hostPageSizeChangeEvent" />
        <!-- 主机列表end -->
      </a-tab-pane>
      <a-tab-pane key="2">
        <template #title>
        ...
        <!-- 漏洞列表start -->
        <a-table style="height:100%" :columns="vulnerabilityTableColumns" :data="vulnerabilityData" column-resize
          :pagination="false" @page-change="handleHostVulnerabilityTablePageChange">
        ...
        <a-pagination :style="{ marginTop: '20px', float: 'right' }" :total="vulnerabilityPagination.total"
          :current="vulnerabilityPagination.pageIndex" :page-size="vulnerabilityPagination.pageSize" show-total
          show-jumper show-page-size @change="vulnerabilityPageIndexChangeEvent"
