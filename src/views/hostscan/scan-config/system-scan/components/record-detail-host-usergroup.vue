<template>
  ...
  <a-table :columns="tableColumns" :data="tableData" column-resize :pagination="false"
    ...
    <template #users="{ record }">
      <a-tag v-for="item in record.users" :key="item"> {{ item }}</a-tag>
    </template>
...
import { getHostUsersList } from '@/api/scan/scan-record';
...
const tableColumns = reactive([
  {
    title: t('scan.record.host.userGroup.name'),
    dataIndex: 'name',
  },
  {
    title: t('scan.record.host.userGroup.users'),
    dataIndex: 'users',
    slotName: 'users'
  }
...
const initTableData = async () => {
  const response = await getHostUsersList(
    hostId,
    pagination.value
  );
