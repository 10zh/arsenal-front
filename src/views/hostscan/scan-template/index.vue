    ...
    <a-card class="general-card" :title="$t('menu.list.scan.template')">
      <a-row ref="searchRef">
        <a-col :flex="1">
          <a-form :model="formModel" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }" label-align="left">
            <a-row :gutter="16">
              <a-col :span="8">
                <a-form-item field="templateName" :label="t('form.search.templateName')">
              ...
              <a-col :span="8">
                <a-form-item field="enableIcmp" :label="t('form.search.enableIcmp')">
              ...
              <a-col :span="8">
                <a-form-item field="isLocal" :label="t('form.search.enableArp')">
      ...
      <a-table row-key="id" :columns="columns" :bordered="false" :data="tableData" :row-selection="rowSelection"
        ...
        <template #operations="{ record }">
          ...
          <a-button type="text" size="small" style="padding: 0px" @click="handleListener('detail', record)">
            <template #icon>
              <icon-menu />
            </template>
            {{ $t('button.detail') }}
...
onMounted(async () => {
  ...
  try {
    const response = await getScanTemplates({ pageIndex: pagination.value.pageIndex, pageSize: pagination.value.pageSize });
    const pageInfo = {
    ...
    response.data.forEach(item => {
      ...
      item.hostDiscovery.enableArp = item.hostDiscovery.enableArp === '1' ? enable : disable;
      item.hostDiscovery.enableIcmp = item.hostDiscovery.enableIcmp === '1' ? enable : disable;
      item.hostDiscovery.enablePu = item.hostDiscovery.enablePu === '1' ? enable : disable;
      item.hostDiscovery.enableSyn = item.hostDiscovery.enableSyn === '1' ? enable : disable;
